...
public class ExampleMain {
	...
	private static void parseInFileOption(CommandLine line)
			throws ParseException {
		_inFile = DEFAULT_READ_DIRECTORY
				+ line.getOptionValue("i", DEFAULT_INI_FILE);
		if (_inFile == null) {
	...
	private static void parseOutFileOption(CommandLine line)
			throws ParseException {
		_outFile = DEFAULT_WRITE_DIRECTORY
				+ line.getOptionValue("o", DEFAULT_OUT_FILE);
	}
	/**
	...
	private static void startBatchMode() throws SimulationFailedException {
		try {
			Controller controller = new Controller(_inFile, _outFile,
					_timeLimit);
			controller.leerDatosSimulacion();
			controller.run();
		} catch (Exception e) {
			e.printStackTrace();
			throw new SimulationFailedException(_inFile, _outFile, _timeLimit,
					e);
