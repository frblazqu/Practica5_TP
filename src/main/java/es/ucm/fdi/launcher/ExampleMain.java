...
/**
 * Las responsabilidades de esta clase es generar un punto de comunicación del
 * usuario con todo el programa y facilitar test y pruebas de todo el
 * conglomerado del simulador (unitarias y múltples).
 * 
...
public class ExampleMain {
	...
	private static void test(String inFile, String outFile, String expectedOutFile,
		...
		try {
			...
			System.out.println("Result for: '"
					+ _inFile.substring(_inFile.lastIndexOf('\\') + 1) + "':\n" + "-> "
					+ (equalOutput ? "OK!" : ("FAIL! :(")));
	...
	private static void start(String[] args) {
		try {
			...
			System.err
					.println("Se ha producido un fallo durante el parseo de argumentos de entrada:\n"
							+ args);
		}
	}
