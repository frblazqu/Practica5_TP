...
public class NewCar extends NewVehicle {
	public static class NewCarBuilder extends NewVehicle.NewVehicleBuilder {
		...
		protected Event leerAtributosEspecificos(IniSection sec) {
			...
			int mFDur = StringParser
					.parseIntValue(sec.getValue("max_fault_duration"));

			if (sec.getValue("seed") != null) {
				long seed = Long.parseLong(sec.getValue("seed"));
				return new NewCar(resist, fProb, mFDur, seed, time, id, mSpeed,
						it);
			} else
	...
	public NewCar(int res, double fProb, int mFDur, long s, int time,
			String vId, int mSpeed, String[] it) {
	...
	public NewCar(int res, double fProb, int mFDur, int time, String vId,
	...
	public void execute(RoadMap map) throws IllegalArgumentException {
		if (!map.duplicatedId(this.getId())) {
			...
			if (validIds) {
				...
				Car car = new Car(resistance, fault_probability,
						max_fault_duration, seed, this.getId(),
						this.getmSpeed(), this.getItinerary(), map);
				map.addVehicle(car);
			}
		} else {
			throw new IllegalArgumentException(
