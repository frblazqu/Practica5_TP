...
public class NewRoad extends Event {
	...
	public NewRoad(int time, String id, String iniId, String destId, int l,
			int mSpeed) {
		super(time);
		road_id = id;
		junctionIniId = iniId;
	...
	public void execute(RoadMap map) throws IllegalArgumentException {
		if (map.duplicatedId(road_id))
			throw new IllegalArgumentException(
					"The id " + road_id + " is already used");
		...
		try {
			...
			if (map.validJuctionsForRoad(junctionIniId, junctionDestId)) {
				...
				ConexionCruces conJunct = new ConexionCruces(road_id,
						junctionDestId);
		...
		public Event parse(IniSection sec) throws IllegalArgumentException {
			if (!sec.getTag().equals("new_road")) {
				...
				try {
					...
					int mSpeed = StringParser
							.parseIntValue(sec.getValue("max_speed"));
					...
					if (sec.getValue("type") == null)
						...
						int lanes = StringParser
								.parseIntValue(sec.getValue("lanes"));
						return new NewFreeway(tm, id, src, dest, l, mSpeed,
								lanes);
					} else
						return null;
				} catch (IllegalArgumentException e) {
					throw new IllegalArgumentException(
							"There is something wrong with one of the atributes.",
							e);
				}
			}
		}
