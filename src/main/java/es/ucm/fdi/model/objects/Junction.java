...
public class Junction extends SimulatedObject {
	...
	public void avanza() {
		if (numCarreterasEntrantes > 0) {
			// Aquí deberíamos tener tal vez un método para inicializarlo o
			// inicializarlo antes.
			if (semaforo == -1)
				semaforo = numCarreterasEntrantes - 1;

			// Tal vez no necesitamos el método colaEnVerde() sino algo más
			// encapsulado.
	...
	protected String colaCruce() {
		...
		for (int i = 0; i < incomingRoadIds.size(); i++) {
			cola += "(" + incomingRoadIds.get(i) + ","
					+ (i == semaforo ? "green," : "red,") + "[" + vehiculosCola(i)
					+ "]),";
		}
